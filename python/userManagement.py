...
from database import doDatabaseQuery, passwordRandomKey
...
def verifyDbUser(username):
    userExists = False
    userTableTuple = doDatabaseQuery("Select username from userTable;")
...
def verifyUserPassword(username, password):
    ...
    if isUser == True:
        dbPassword = doDatabaseQuery("Select password from userTable WHERE username='{0}'".format(username))[0]
...
#         camPerms = doDatabaseQuery("Select camPermissions from userTable WHERE username='{0}'".format(username))
