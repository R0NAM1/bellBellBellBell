...
var videosLoading = 0;
var dynVidArray = []
// This is a bit of an odd one, I need to track if all the clips segment 5 has loaded 
var loadedSegmentsArray = []
...
class DynamicM4SVideoPlayer {
    ...
    async loadSegment(index, startTime) {
        try {
            ...
            // thD 33f36e
            thD.style.backgroundColor = '#33f36e';


            for (var element of allBelow) {
                console.log(element)
                element.style.backgroundColor = '#759200';
            }
...
export function generateTimelineEmpty(fromDate, toDate, camListArray){
    ...
    playerControls.style.height = '165px';
    ...
    if (allowToPost == true) {
        ...
        fetch('/search/m3u8request',
        {
            ...
            .then(data => {                
                ...
                bottomTimelineControls.style.height = '164px';
                bottomTimelineControls.style.top = '-25px';
                playerControls.style.top = '-26px';
                step1selectcameras.style.height = 'calc(100vh -  455px)';
                step1selectcameras.style.top = '30px';
...
async function onFinishedLoading() {
    ...
    collapseButtonElement.addEventListener('click', () => {
        ...
        leftSideCollapseButton.style.top = '-80px';
